  ...
  export default {
    ...
    methods: {
      ...
      addData () {
        ...
        this.id = "";
        let fn = function (response, self) {
          self.osDatas = response.data.result.ostypes;
          self.updateDatas = response.data.result.upgrade_types;
          self.versionDatas = response.data.result.versions;
          self.supportDatas = response.data.result.versions;
        };
        // 查询下拉框信息
      ...
      apkFormConfirm () {
        ...
        if (this.$refs.upload && this.$refs.upload.uploadFiles) {
          ...
          this.dataParam.product_type_id = 1000;
