  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>待结佣</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-table :data="orderList" v-loading="listLoading" style="width: 100%" border>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="small" @click="handleContract(scope.$index, scope.row)">完成合同</el-button>
          <el-button size="small" type="danger" @click="handleSettled(scope.$index, scope.row)">完成结佣</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      handleSearch() {
        ...
        this.$axios.get('/order/orderlist', {params: {status: '120', startDate: this.startDate, endDate: this.endDate, pageNumber: this.pageNumber, pageSize: this.pageSize}}).then((res) => {
      ...
      handleContract(index, row) {
        this.$confirm('确认该订单已完成合同?', '提示', {
          ...
          this.$axios.post('/order/ordercontract', {uid: row.uid}).then((res) => {
            if (res.data.status == 200) {
              this.$message({
                type: 'success',
                message: '操作成功!'
      ...
      handleSettled(index, row) {
        this.$confirm('确认该订单已结佣?', '提示', {
          type: 'warning'
        }).then(() => {
          this.$axios.post('/order/ordersettled', {uid: row.uid}).then((res) => {
            if (res.data.status == 200) {
              this.$message({
                type: 'success',
                message: '操作成功!'
