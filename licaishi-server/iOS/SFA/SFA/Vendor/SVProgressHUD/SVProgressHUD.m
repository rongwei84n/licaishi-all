...
+ (void)showSuccessWithStatus:(NSString*)status {
    ...
    [[AppDelegate sharedAppDelegate].window makeToast:status duration:1.0 position:CSToastPositionBottom];
...
//李满改
+ (void)showCustomStatus:(nullable NSString*)status
{
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showImage:nil status:status duration:displayInterval];
}
//李满改
+ (void)showCustomStatus:(nullable NSString*)status maskType:(SVProgressHUDMaskType)maskType
{
    SVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
