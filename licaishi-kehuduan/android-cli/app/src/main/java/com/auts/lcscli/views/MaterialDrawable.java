...
class MaterialDrawable extends RefreshDrawable implements Animatable {
    ...
    private void createCircleDrawable() {
        ...
        final float density = PhApplication.getAppContext().getResources().getDisplayMetrics().density;
...
//        Paint mPaint = new_icon Paint();
//        mPaint.setColor(mContext.getResources().getColor(R.color.text_oringe));
//        Drawable drawable = mContext.getResources().getDrawable(R.drawable.get_wifi_loading);
//        c.drawBitmap(getMyBitmap(drawable), 30, 30, mPaint);
        c.restoreToCount(saveCount);
    }

    static Bitmap getMyBitmap(Drawable drawable) {  // drawable 转换成bitmap
        int width = drawable.getIntrinsicWidth();// 取drawable的长宽
        int height = drawable.getIntrinsicHeight();
        Bitmap.Config config = drawable.getOpacity() != PixelFormat.OPAQUE ? Bitmap.Config.ARGB_8888 : Bitmap.Config.RGB_565;// 取drawable的颜色格式
        Bitmap bitmap = Bitmap.createBitmap(500, 500, config);// 建立对应bitmap
        return bitmap;
    }
    ...
    private void setupAnimators() {
        ...
        final Animation animation = new Animation() {
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                final float startTrim = startingTrim + (MAX_PROGRESS_ARC * END_CURVE_INTERPOLATOR
                ...
                float groupRotation = ((720.0f * 4 / NUM_POINTS) * interpolatedTime)
                        + (720.0f * 4 * (mRotationCount / NUM_POINTS));
    ...
    private static class Ring {
        ...
        public Ring(Callback callback, Context context) {
            mCallback = callback;
            mContext = context;
...
//        }
        ...
        // TODO: 2017/9/11 自己重写的动画逻辑
        public void draw(Canvas c, Rect bounds) {
            final RectF arcBounds = mTempBounds;
            arcBounds.set(bounds);
            arcBounds.inset(mStrokeInset, mStrokeInset);

            Bitmap bitmap = ((BitmapDrawable) mContext.getResources().getDrawable(R.drawable.get_wifi_loading)).getBitmap();
            Rect srcRect = new Rect(0, 0, bitmap.getWidth(), bitmap.getHeight());
            Rect desRect = new Rect(0, 0, bitmap.getWidth(), bitmap.getHeight());
            c.drawBitmap(bitmap, null, arcBounds, mCirclePaint);
        }
